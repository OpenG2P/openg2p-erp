...
class BatchExportXlsx(models.AbstractModel):
    ...
    def _get_ws_params(self, wb, data, batch):
        if len(batch) > 1:
            raise ValidationError(_("This report can only be generated with one disbursement batch at a time"))

        spec = self._batch_report_colspec()
        cols = [i[0] for i in sorted([(key, value['sequence']) for key, value in spec.items()], key=lambda x: x[1])]

        return [{
            'ws_name': (batch.name[:28] + '..') if len(batch.name) > 30 else batch.name,
    ...
    def _get_columns(self):
        ...
        sequence = 100
        for rule in self.env['openg2p.disbursement.rule'].search([('appears_on_slip', '=', True)]):
            template[rule.code] = {
                'header': {
                    'value': rule.name,
                },
    ...
    def _batch_report(self, workbook, ws, ws_params, data, batch):
        ...
        row_pos = 0
        if len(batch) == 1:
            ws_params['title'] = batch.name + " - Draft Report" if batch.state_approved else "Report"
        ...
        for slip in batch.slip_ids:
            ...
            for line in slip.details_by_disbursement_rule_category:
                slip_data[line.disbursement_rule_id.code] = formatLang(self.env, line.total,
                                                                       currency_obj=slip.currency_id)
