...
class ContributionRegisterReport(models.AbstractModel):
    ...
    def _get_slip_lines(self, register_ids, batch_ids):
        ...
        self.env.cr.execute("""
            ...
            AND hp.state = 'done'
            ORDER BY pl.slip_id, pl.sequence""", (tuple(batch_ids), tuple(register_ids)))
        line_ids = [x[0] for x in self.env.cr.fetchall()]
        for line in self.env['openg2p.disbursement.slip.line'].browse(line_ids):
            result.setdefault(line.register_id.id, self.env['openg2p.disbursement.slip.line'])
    ...
    def _get_report_values(self, docids, data=None):
        ...
        contrib_registers = self.env['openg2p.disbursement.contribution.register'].browse(register_ids)
