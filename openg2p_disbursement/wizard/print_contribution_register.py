...
class PrintBatchContribRegister(models.TransientModel):
    ...
    def print_contrib_register(self):
        ...
        if not batch.state_approved():
            raise ValidationError(_("Batch has tp be approved before registers can be printed"))
        ...
        return self.env.ref('openg2p_disbursement.action_contribution_register').report_action(
            self.register_ids, data=datas)
