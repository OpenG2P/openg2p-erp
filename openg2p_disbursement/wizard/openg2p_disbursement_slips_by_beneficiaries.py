...
class HrSlipBeneficiaries(models.TransientModel):
    ...
    def _add_program_domain(self):
        ...
        beneficiaries = self.env['openg2p.disbursement.batch'].browse(active_id).default_run_beneficiaries()
        return [('id', 'in', list(set(beneficiaries) - set(batch.slip_ids.mapped('beneficiary_id.id'))))]
    ...
    def compute_sheet(self):
        self.ensure_one()
        active_id = self.env.context.get('active_id')
        res = self.env['openg2p.disbursement.batch'].browse(active_id).generate_run(
            beneficiaries=self.beneficiary_ids.ids, redo=True)
