...
class Openg2pDisbursementRule(models.Model):
    ...
    parent_rule_id = fields.Many2one(
        ...
        string='Parent Disbursement Rule',
        index=True
    )
    ...
    def _check_parent_rule_id(self):
        if not self._check_recursion(parent='parent_rule_id'):
            raise ValidationError(_('Error! You cannot create recursive hierarchy of Disbursement Rules.'))
    ...
    def _compute_rule(self, localdict):
        ...
        if self.amount_select == 'fix':
            try:
                ...
                raise UserError(_('Wrong quantity defined for disbursement rule %s (%s).') % (self.name, self.code))
            ...
            try:
                ...
                raise UserError(_('Wrong percentage base or quantity defined for disbursement rule %s (%s).') % (
                    self.name, self.code))
        else:
            try:
                ...
                raise UserError(_('Wrong python code defined for disbursement rule %s (%s).') % (self.name, self.code))
    ...
    def _satisfy_condition(self, localdict):
        ...
        if self.condition_select == 'none':
            ...
            try:
                ...
                raise UserError(
                    _('Wrong range condition defined for disbursement rule %s (%s).') % (self.name, self.code))
        else:  # python code
            try:
                ...
                raise UserError(
                    _('Wrong python condition defined for disbursement rule %s (%s).') % (self.name, self.code))
