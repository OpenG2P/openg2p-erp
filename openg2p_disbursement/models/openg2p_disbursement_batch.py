...
class SlipBatch(models.Model):
    ...
    def _compute_can_generate(self):
        self.ensure_one()
        self.can_generate = self.state == 'draft' and not isinstance(self.id, models.NewId)

    @api.multi
    def _compute_can_cancel(self):
        self.ensure_one()
        self.can_cancel = self.state in ('generating', 'draft', 'confirm', 'approved') and not isinstance(self.id,
                                                                                                          models.NewId)

    @api.multi
    def _compute_can_confirm(self):
        self.ensure_one()
        self.can_confirm = self.state in ('generated', 'draft') and len(self.slip_ids) and (
                self.job_batch_id and (self.job_state == 'finished' and self.job_failed_count == 0) or True
        )
    ...
    @api.multi
    @api.depends('job_batch_id', 'job_batch_id.completeness', 'job_batch_id.failed_percentage')
    ...
    def confirm_slip_run(self):
        if not self.slip_ids:
            raise ValidationError(_("Disbursement slips should be generated before batch can be confirmed"))
    ...
    def _compute_exception_count(self):
        for rec in self:
            rec.exception_count = self.env['openg2p.disbursement.exception'].search_count([('batch_id', '=', rec.id)])
    ...
    def ommitted_beneficiaries_check(self, beneficiaries):
        ...
        existing = set(self.env['openg2p.disbursement.exception'].search(
            [('batch_id', '=', self.id)]).mapped('beneficiary_id.id')
        )
