...
class Openg2pDisbursementReconciliation(models.TransientModel):
    ...
    def _default_previous_batch(self):
        ...
        previous_batch = batch_obj.search(
            [('date_start', '<', current_batch.date_start), ('program_id', '=', current_batch.program_id.id)],
    ...
    def _constraint_program(self):
        for rec in self:
            if rec.current_batch_id.program_id != self.previous_batch_id.program_id:
                raise ValidationError(_("Batches being compared should be of the same program"))
    ...
    def _get_modifications(self):
        ...
        for beneficiary in additions:
            line = self.current_batch_id.slip_ids.filtered(lambda r: r.beneficiary_id == beneficiary) \
                .line_ids.filtered(lambda r: r.disbursement_rule_id.code == "GROSS")
            modifications.append(
                ('Addition', beneficiary.display_name, beneficiary.ref, beneficiary.location_id.display_name,
                 'GROSS', line.amount, 0.0)
            )
        for beneficiary in removals:
            line = self.previous_batch_id.slip_ids.filtered(lambda r: r.beneficiary_id == beneficiary) \
                .line_ids.filtered(lambda r: r.disbursement_rule_id.code == "GROSS")
            modifications.append(
                ('Removal', beneficiary.display_name, beneficiary.ref, beneficiary.location_id.display_name,
                 'GROSS', 0.0, line.amount)
            )

        disbursement_rules = (
                self.previous_batch_id.slip_ids.mapped('line_ids.disbursement_rule_id')
                |
                self.current_batch_id.slip_ids.mapped('line_ids.disbursement_rule_id')
        )
        disbursement_rules = disbursement_rules.filtered(lambda r: r.category_id.code in ('BASE', 'ADD'))

        for beneficiary in retained:
            prev_slip = self.previous_batch_id.slip_ids.filtered(lambda r: r.beneficiary_id == beneficiary)
            current_slip = self.current_batch_id.slip_ids.filtered(lambda r: r.beneficiary_id == beneficiary)

            for rule in disbursement_rules:
                previous_line = prev_slip.line_ids.filtered(lambda r: r.disbursement_rule_id == rule)
                current_line = current_slip.line_ids.filtered(lambda r: r.disbursement_rule_id == rule)
                ...
                if abs(prev_amount - current_amount) > 0.5:
                    modifications.append(
                        ('Modification', beneficiary.display_name, beneficiary.ref,
                         beneficiary.location_id.display_name, rule.code, current_amount, prev_amount)
                    )
    ...
    def print_report(self):
        ...
        return self.env.ref('openg2p_disbursement_reconciliation.action_report_reconcile').report_action(
