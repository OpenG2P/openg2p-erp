...
class Openg2pDisbursementBatch(models.Model):
    ...
    def close_slip_run(self):
        self.env['openg2p.disbursement.amendment'].search(
            [('batch_id', '=', self.ids), ('state', '=', 'validate')]).action_done()
        super(Openg2pDisbursementBatch, self).close_slip_run()

    @api.multi
    def _compute_amendment_count(self):
        for rec in self:
            rec.amendment_count = self.env['openg2p.disbursement.amendment'].search_count(
                [('batch_id', '=', rec.id), ('state', '!=', 'cancel'), ('program_id', '=', rec.program_id.id)])
