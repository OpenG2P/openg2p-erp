        ...
        for rec in self:
            ...
            if self.env['openg2p.disbursement.rule'].search_count(domain) > 0:
                raise ValidationError(
                    'Amendment category code  %s conflicts with a disbursement rule code.' % (rec.code,))
            ...
            if self.env['openg2p.disbursement.rule.input'].search_count(domain) > 0:
                raise ValidationError('Amendment category code %s conflicts with an input rule code.' % (rec.code,))
        return True
