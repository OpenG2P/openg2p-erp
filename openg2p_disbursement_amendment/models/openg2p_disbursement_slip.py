...
class Openg2pDisbursementSlip(models.Model):
    ...
    def _compute_amendment_count(self):
        for rec in self:
            rec.amendment_count = self.env['openg2p.disbursement.amendment'].search_count([('slip_id', '=', rec.id)])
    ...
    def compute_sheet(self):
        ...
        for slip in self:
            for input in slip.input_line_ids:
                input_rule = self.env['openg2p.disbursement.rule.input'].search([('code', '=', input.code)])
    ...
    def reset_amendments(self):
        self.env['openg2p.disbursement.amendment'].search([('slip_id', 'in', self.ids)]).action_reset()
