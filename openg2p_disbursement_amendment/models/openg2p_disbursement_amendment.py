...
class Openg2pDisbursementAmendment(models.Model):
    ...
    def constrain_batch(self):
        ...
        if self.filtered(lambda r: r.state == 'draft' and (
                r.batch_id.state == 'draft' or r.batch_id.slip_count != 0)):
            raise ValidationError(_("Batch has to be in the draft state with no computed slip to be selected here"))

    @api.onchange('beneficiary_id')
    @api.multi
    def _constraint_amendment(self):
        for rec in self:
            if not rec.program_id and rec.beneficiary_id and len(rec.beneficiary_id.program_ids) == 1:
    ...
    def _constraint_check_enrollment(self):
        for rec in self:
            if rec.program_id not in rec.beneficiary_id.program_ids:
                raise ValidationError("Beneficiary %s is not registered to program %s"
    ...
    def onchange_beneficiary(self):
        for rec in self:
            if rec.beneficiary_id and rec.category_id:
                rec.name = _('%s Amendment for %s') % (rec.category_id.name, rec.beneficiary_id.display_name)
    ...
    def action_reset(self):
        for rec in self:
            if rec.state == 'done':
                raise ValidationError(_("You cannot reset amendments in the drat stage"))
