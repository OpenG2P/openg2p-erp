...
def _post_init_hook_add_trgm_indexes(cr, registry):
    with cr.savepoint():
        ...
        if not trgm_mod.index_exists('openg2p.registration', 'firstname'):
            field_name = env.ref('openg2p_registration.field_openg2p_registration__firstname')
            trgm_mod.create({
        ...
        if not trgm_mod.index_exists('openg2p.registration', 'lastname'):
            field_name = env.ref('openg2p_registration.field_openg2p_registration__lastname')
            trgm_mod.create({
                'field_id': field_name.id,
