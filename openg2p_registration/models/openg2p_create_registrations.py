...
class Registration(models.Model):
    ...
    def create_registration_from_odk(self, odk_data):
        ...
        try:
            regd = self.create(
                {
                    ...
                    "stage_id": (temp["stage_id"] if "stage_id" in temp.keys() else 1),
            ...
            rid = regd.id
            self.create_disbursement_fields(
                rid, temp, regd, demo_auth_res, should_merge, should_create_beneficiary
            )
    ...
    def create_disbursement_fields(
        self, rid, temp, regd, demo_auth_res, should_merge, should_create_beneficiary
    ):
        data = {}
        odk_data = temp
        org_data = {}
