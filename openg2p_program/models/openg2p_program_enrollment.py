...
class ProgramEnrollment(models.Model):
    ...
    def create(self, vals):
        if vals.get('name', False) in ('/', False):
            vals['name'] = self.env['ir.sequence'].next_by_code('openg2p.program.enrollment.ref')
        ...
        for c in self:
            if c.date_end and c.date_start > c.date_end:
                raise ValidationError(
                    _('Program Enrollment start date must be earlier than program enrollment end date.'))

            if c.date_start > c.program_id.date_start:
                raise ValidationError(
                    _('Program Enrollment start date must not be earlier  than program\'s start date.'))

            if c.date_end and c.date_end > c.program_id.date_end:
                raise ValidationError(
                    _('Program Enrollment end date must be later than program end date.'))
