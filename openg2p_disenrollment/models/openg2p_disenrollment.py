...
class Openg2pDisenrollment(models.Model):
    _name = 'openg2p.disenrollment'
    _description = 'Beneficiary Disenrollment'
    _order = "name DESC"
    ...
    def _constraint_beneficiary_id(self):
        for rec in self:  # TODO performance improve
            if not rec.beneficiary_id.active:
                raise ValidationError(_("%s is already inactive"
                                        % (rec.beneficiary_id.display_name,)))

            if self.search_count([('beneficiary_id', '=', rec.beneficiary_id.id), ('state', 'in', ('draft', 'confirm')),
                                  ('id', '!=', rec.id)]):
                raise ValidationError(_("An active disenrollment record already exists for %s"
                                        % (rec.beneficiary_id.display_name,)))

    @api.model
    @api.returns('self', lambda value: value.id)
    def create(self, vals):
        res = super(Openg2pDisenrollment, self).create(vals)
    ...
    def state_done(self):
        ...
        for termination in self:
            if today < termination.date:
                raise UserError('Unable to complete operation as the effective date is still in the future!')
                ...
                termination.beneficiary_id.program_registration_ids.filtered(
                    lambda r: r.program_id == termination.program_id and r.state in ('open', 'draft')).write(
                    {'date_end': termination.date, 'state': 'close'})
