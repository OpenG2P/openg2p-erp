...
class Bank(models.Model):
    ...
    def _selection_execute_method(self):
        return [
            ('manual', _('Manual')),
            ('server2server', _('Server-to-Server'))
        ]
    ...
    def _selection_provider(self):
        if self._context.get('install_mode'):
            ...
            builder.build_registry(components_registry, states=('installed', 'to upgrade', 'to install'))
        return [(p._provider_name, p._provider_name.title())
                for p in self.env['openg2p.gateway.transaction'].get_all_providers]

    def _get_allowed_execute_method(self):
        return self.env['openg2p.gateway.transaction'].get_provider._allowed_execute_method
        ...
        if self.provider:
            ...
            if self.execute_method not in execute_methods:
                ...
                return {'warning': {
                    ...
                    'message': _('This method is not compatible with the provider selected'),
                }}
