...
class Openg2pDisbursementSlip(models.Model):
    ...
    def compute_sheet(self):
        """
        do we have an unprocessed disenrollment record for subject with effective date before the end of the batch?
        """
    ...
    def check_pending_denrollments(self):
        ...
        active_disenrolls = self.env['openg2p.disenrollment'].search_read([
            ('beneficiary_id', 'in', self.mapped('beneficiary_id.id')), ('state', '=', 'confirm'),
            ('date', '<', batch.date_end)
        ], ['beneficiary_id', ])
        rule = self.env.ref("openg2p_bridge_disbursement_disenrollment.exception_rule_disenrollment_started")
