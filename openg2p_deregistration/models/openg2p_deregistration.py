...
class Openg2pDeregistration(models.Model):
    ...
    type = fields.Selection(
        [('program', 'Dis-enroll from a Program'), ('database', 'De-register - End all Programs and Archive')],
        required=True
    )
    ...
    def _constraint_beneficiary_id(self):
        for rec in self:  # TODO performance improve
            ...
            if self.search_count([('beneficiary_id', '=', rec.beneficiary_id.id), ('state', 'in', ('draft', 'confirm')),
                                  ('id', '!=', rec.id)]):
                raise ValidationError(_("An active de-registration record already exists for %s"
                                        % (rec.beneficiary_id.display_name,)))
