...
class Openg2pDisbursementBatch(models.Model):
    ...
    def close_slip_run(self):
        if self.advice_ids.filtered(lambda r: r.state not in ('done', 'cancel')):
            raise ValidationError(_("All advices should be either in the done or canceled state before batch can be"
                                    " closed"))
    ...
    def create_advice(self):
        ...
        if self.advice_ids.filtered(lambda r: r.state != 'draft'):
            raise UserWarning(
                _("Cannot regenerate of an advice in a batch is past the draft stage") % self.name)
